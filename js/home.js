...
const pintarProductos = (data) => {
    ...
    data.forEach(cubierta => {
      ...
      div.innerHTML += `
          ...
          <div class="card-carrito d-flex justify-end">  
              <a class="btn-floating halfway-fab wabes-effect waves-light red">
                  <i data-cubierta-id="${cubierta.id}" class="material-icons agregar">add_shopping_cart</i>
              </a>
          </div>
      `;
      contenedor.appendChild(div);
        ...
        const btnComprar = document.getElementById(`medida${cubierta.id}`)
        btnComprar.addEventListener("click",()=> {
            validarProductoRepetido(data,cubierta.id,select.value)
        })
    });
